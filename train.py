...
class TrainPipeline:
    ...
    def __init__(self, init_model=None):
        ...
        self.lr_multiplier = 1  # Learning rate multiplier adapted via KL
        self.temp = 1.0
        self.batch_size = CONFIG['batch_size']  # Training batch size
        self.epochs = CONFIG['epochs']  # Train steps per update
        self.kl_targ = CONFIG['kl_targ']  # KL-divergence target
        self.check_freq = 100  # Model checkpoint frequency
        self.game_batch_num = CONFIG['game_batch_num']  # Number of updates
        ...
        if init_model:
            try:
                ...
                print('Loaded the last final model.')
            except:
                # Train from scratch if path missing
    ...
    def policy_evaluate(self, n_games=10):
        ...
        Note: this is for monitoring only.
        """
    ...
    def policy_updata(self):
        ...
        # Recover numpy arrays if states/probs were zipped
        ...
        for i in range(self.epochs):
            ...
            kl = np.mean(np.sum(old_probs * (
                np.log(old_probs + 1e-10) - np.log(new_probs + 1e-10)),
                                axis=1))
            # Early stop if KL diverges too much
            if kl > self.kl_targ * 4:
                break

        # Adapt learning rate based on KL
    ...
    def run(self):
        ...
        try:
            for i in range(self.game_batch_num):
                ...
                # Update model every train_update_interval seconds (e.g., 10 minutes)
                time.sleep(CONFIG['train_update_interval'])

                if (i + 1) % self.check_freq == 0:
                    # Optional: evaluate and manage best policy
